...
def memory_db(monkeypatch):
    ...
    # Use 23:59 to avoid time-of-day failures during general tests
    conn.execute(f"INSERT INTO showings (screen_id, show_date, show_time, seats_remaining, is_cancelled) VALUES (1, '{today_str}', '23:59', 100, 0)")
    conn.execute(f"INSERT INTO showings (screen_id, show_date, show_time, seats_remaining, is_cancelled) VALUES (2, '{today_str}', '23:59', 100, 0)")
...
def test_validate_booking_date_past():
    ...
    from src.models.booking import BookingError
...
def test_validate_booking_date_valid():
    ...
    # Today should be valid for a late show time
    today = datetime.date.today()
    assert BookingManager.validate_booking_date(today, "23:59") is None
...
def test_duplicate_booking(memory_db):
    ...
    # Insert a showing for today at 23:59 to ensure it hasn't passed
    memory_db.execute("INSERT INTO showings (film_id, screen_id, show_date, show_time, show_type, seats_remaining) VALUES (1, 1, ?, '23:59', 'evening', 100)", (today_str,))
    ...
    # Check for duplicate
    dup = BookingManager.check_duplicate("john@example.com", 1, today, memory_db)
    assert dup is not None
    assert dup["booking_ref"] == res1["booking_ref"]
    
    # Check for non-duplicate
    no_dup = BookingManager.check_duplicate("jane@example.com", 1, today, memory_db)
