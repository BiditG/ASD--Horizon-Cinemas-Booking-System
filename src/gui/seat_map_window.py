...
class SeatMapWindow:
    def __init__(self, parent: tk.Toplevel, showing_id: int, required_quantity: int,
                 ticket_type: str, on_confirm: Callable[[List[str]], None]):
    ...
    def _build_ui(self):
        ...
        tk.Label(legend_frame, text="Legend:", bg=BG, fg=TEXT, font=(
            FONT_FAMILY, 12, "bold")).pack(side="left", padx=10)
        ...
        self.status_lbl = tk.Label(
            header_frame, text=f"0 / {self.required_quantity} seats selected", bg=BG, fg=TEXT, font=(FONT_FAMILY, 14))
        ...
        self.cancel_btn = tk.Button(self.btn_frame, text="Cancel", bg="#26344a", fg="white", font=(
            FONT_FAMILY, 12, "bold"), padx=15, command=self.root.destroy, relief="flat")
        self.cancel_btn.pack(side="left", padx=20)

        self.manual_btn = tk.Button(self.btn_frame, text="Choose Manually", bg=WARNING, fg="black", font=(
            FONT_FAMILY, 12, "bold"), padx=15, command=self._enable_manual, relief="flat")
        ...
        for zone, seat_num in all_seats:
            ...
            btn = tk.Button(self.scrollable_frame, text=seat_num, width=4, height=2, bg=color,
                            fg="white" if not is_booked else TEXT2, font=(FONT_FAMILY, 10, "bold"), relief="flat")
            if is_booked:
    ...
    def _toggle_seat(self, seat_num: str, original_color: str):
        ...
        if data["zone"] != self.ticket_type:
            messagebox.showwarning(
                "Invalid Zone", f"You must select seats in the {self.ticket_type.replace('_', ' ').title()} zone.")
            return
